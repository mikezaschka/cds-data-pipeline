...
| **`PIPELINE.*` event hooks** — minimal-effort customization of any phase | Filter, enrich, normalize, forward. `before` / `on` / `after` on `PIPELINE.READ / MAP / WRITE` in classic CAP style. | [Event hooks](event-hooks.md) |
