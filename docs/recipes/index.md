...
| **`PIPELINE.WRITE` hook override** — one-off forwarding, not worth a reusable adapter | Quick prototypes; pipelines where the write shape is bespoke and will not be reused. | [Write-hook override](write-hook-override.md) |
...
| **In-process queued scheduler** — persistent, cross-instance single-winner schedule owned by the CAP app | Scaled deployments (>1 app instance) that want persistence, retry, and cross-instance safety without an external service. `schedule: { every, engine: 'queued' }`. | [Internal scheduling (queued)](internal-scheduling-queued.md) |
...
flowchart TD
  ...
  tgt -->|"One-off forwarding"| tgtHook[Override PIPELINE.WRITE]
  ...
  tgtHook --> hookRecipe["Recipe: Write-hook override"]
  srcCustom --> customSrc["Recipe: Custom source adapter"]
```

## Use-case labels

The older "replicate / materialize / move-to-service" labels still show up in the codebase and logs as doc-level vocabulary — they describe common *combinations* of read shape and target destination, but they are not a field on `addPipeline(...)` and not a column on the tracker. The engine dispatches through source / target adapter factories. See [Concepts → Inference rules](../concepts/inference.md).
...
| **Move-to-service** | Entity-shape read, non-DB target | [Built-in replicate → OData target](built-in-replicate.md#to-a-remote-odata-target) · [Custom target adapter](custom-target-adapter.md) · [Write-hook override](write-hook-override.md) |
